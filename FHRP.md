# Résilience Réseau avec FHRP (HSRP)[^1] 

[^1]: Document réalisé par Amolitho Balde.
...
| ---------- | ----------------- | ------------ |
| PC2        | Switch 2 (Fa0/3)  | 192.168.2.20 |
| R1 (Fa0/0) | Switch  2 (Fa0/2) | 192.168.2.2  |
| R2 (Fa0/0) | Switch 2 (Fa0/3)  | 192.168.2.3  |
...
| Équipement | Connecté à        | Adresse IP   |
| ---------- | ----------------- | ------------ |
| PC1        | Switch 1 (Fa0/3)  | 192.168.1.10 |
| R1 (Fa0/1) | Switch  1 (Fa0/2) | 192.168.1.2  |
| R2 (Fa0/1) | Switch 1 (Fa0/1)  | 192.168.1.3  |
...
| --------- | ------ | ------ | ----------- | ----------- |
| LAN OUEST | actif  | backup | 192.168.1.1 | groupe 1    |
| LAN EST   | backup | actif  | 192.168.2.1 | groupe 2    |
...
- `ping 192.168.1.1` depuis les PCs.
...
**Analyse HSRP avec `show standby brief`** sur R2 :

1. Sur **Fa0/0** de R2, le routeur est en **Standby** dans le groupe 1, car R1 (192.168.2.2) a une **priorité plus élevée**. 

- Sur **Fa0/1 **de R2, le routeur est **Actif** dans le groupe 2, car il a la **priorité la plus haute (110)** et le rôle **preempt** est activé (P).
...
Le résultat de la commande **show standby sur R2** est montré par la capture suivante : ![Résulat de show standby](./Images/show_standby.png)



**Analyse HSRP – Synthèse des rôles et paramètres montrés par la capture (faites sur R2):**

| Interface | Groupe | Connectée à | État sur R2 | IP virtuelle (VIP) | MAC virtuelle  | Routeur actif  | IP du routeur actif | Priorité R2 | Préemption |
| --------- | ------ | ----------- | ----------- | ------------------ | -------------- | -------------- | ------------------- | ----------- | ---------- |
| Fa0/0     | 1      | LAN OUEST   | Standby     | 192.168.2.1        | 0000.0C07.AC01 | R1             | 192.168.2.2         | 90          | Oui        |
| Fa0/1     | 2      | LAN EST     | **Active**  | 192.168.1.1        | 0000.0C07.AC02 | **Local (R2)** | —                   | 110         | Oui        |
